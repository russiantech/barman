...
from web.apis.utils.valid_schemas import (
    signin_schema, signup_schema, msg_schema, update_user_schema, reset_password_schema, 
    reset_password_email_schema, validTokenSchema
)
from web.apis.utils.helpers import ( error_response, success_response, )

from web.models import Transaction, User, Notification
# from web.models import Transaction, User, Task, Order, Notification, AccountDetail
...
def message():
    try:
        ...
        # Validate the data against the schema
        try:
            validate(instance=data, schema=msg_schema)
        except ValidationError as e:
            return jsonify({"success": False, "error": e.message})
        
        # Prepare email content for support
        subject = "[Contact Form] New Message from {}".format(email)
        text_body = f"From: {email} (Phone: {phone})\n\nMessage:\n{message}"
        html_body = render_template('email/contact_message.html', email=email, phone=phone, message=message)
        ...
        emailer.send_email( 
            ...
            # recipients=['support@example.com'],  # The contact email for receiving messages
            recipients=[current_app.config['MAIL_USERNAME']],  # The contact email for receiving messages
            text_body=text_body,
            html_body=html_body
        )

        return success_response(f'Your message has been sent. Thank you!')
