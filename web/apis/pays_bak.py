import secrets
import traceback, requests, sqlalchemy as sa
...
from web.apis.utils import generate_random_id
from web import csrf
from requests.exceptions import ConnectionError, Timeout, RequestException
from jsonschema import validate, ValidationError
from web.apis.utils import success_response, error_response, pay_schema
        ...
        if request.method == "POST":
            ...
            try:
                if subscription:
                    ...
                    # plan_response = requests.request(method="POST", url=plan_init_link, headers=headers, data=plan_payload)
                    # plan_response = requests.request(method="POST", url=plan_init_link, headers=headers, json=plan_payload)
                    # print(plan_response.text)  # Log raw response content
                    # print(plan_response.__dict__)
                    # plan_data = dict(plan_response.json())
                ...
                payment_data = {
                    ...
                    'is_subscription': True,  # Mark as a subscription
...
def transaction_callback():
    try:
        ...
        if not transaction:
            return error_response('Transaction record not found'), 404
        ...
        if status == 'successful':
            ...
            if response.status_code == 200:
                ...
                if (
                    ...
                    """
                    if transaction.plan_id and transaction.user:
                        
                        # Check if the user is already subscribed in the course
                        ...
                        if not existing_enrollment:
                            ...
                            enrollment = Enrollment(
                                user=transaction.user, user_id=transaction.user.id,
                                course=transaction.course, plan_id=transaction.course.id
                            )
                            
                            db.session.add(enrollment)
                            db.session.commit()
                            return success_response('Congrats, your subscription was successful', data=response_data), 200
                        
                        return success_response('Congrats, your subscription was successful', data=response_data), 200
                    """
                   
                    return success_response('Transaction verified successfully', data=response_data), 200
                
                else:
                    return error_response(f'Transaction verification failed->{response_data}'), 400
            else:
                return error_response('Failed to verify transaction'), 400
